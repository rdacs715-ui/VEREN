...
function openBoard(evt, boardName) {
  ...
  // 추가: 해당 board 안의 첫 번째 inner 탭 자동 실행
  let currentBoard = document.getElementById(boardName);

  // 현재 보드 안에 있는 inner 탭 버튼 찾기
  let firstInnerTab = currentBoard.querySelector(".inner_tablink");
  ...
  // evt.currentTarget → 지금 클릭한 요소 (li)
  // 기존 클래스 뒤에 " opacity"를 붙임
...
function openInnerTab(evt, innerName) {
  ...
  // 클릭한 탭이 속한 .board 안에서만 동작하도록 범위 한정
  const currentBoard = evt.currentTarget.closest('.board');

  // 1. 현재 board 안의 inner_board만 숨김
  let boards = currentBoard.getElementsByClassName("inner_board");
  for (let i = 0; i < boards.length; i++) {
    boards[i].style.display = "none";
  }

  // 2. 현재 board 안의 탭 버튼만 active 제거
  let tabs = currentBoard.getElementsByClassName("inner_tablink");
  for (let i = 0; i < tabs.length; i++) {
    tabs[i].className = tabs[i].className.replace(" active", "");
  }

  // 3. 선택한 inner_board 표시 (flex로 열기)
  document.getElementById(innerName).style.display = "flex";
...
function searchFAQ(input) {
  ...
  const activeBoard = document.querySelector('#Board02 .inner_board[style*="flex"]')
    || document.querySelector('#Board02 .inner_board');

  if (!activeBoard) return;

  const items = activeBoard.querySelectorAll('.inner_right > li');

  items.forEach(li => {
    const text = li.querySelector('.question span')?.textContent.toLowerCase() || '';
  ...
  // 페이지네이션 처리
  const numberBox = activeBoard.querySelector('.number');
  if (!numberBox) return;

  if (keyword) {
    // 원본 저장 (최초 1회만)
    if (!numberBox.dataset.original) {
      numberBox.dataset.original = numberBox.innerHTML;
    }
    // 숫자 1만 표시
    numberBox.innerHTML = '<span class="on">1</span>';
  } else {
    // 검색어 없으면 원본 복원
    if (numberBox.dataset.original) {
      numberBox.innerHTML = numberBox.dataset.original;
      delete numberBox.dataset.original;
    }
  }
}